...
const AI_MODEL_ID = 'f3447d4f-ffdb-4f48-aa60-dc9640ccd879'; // Grok 4.1 Fast
...
async function sendDmMessage() {
  ...
  dmHistories[currentDmUser.name].push({ text, time: 'just now', mine: true });
  renderDmMessages();
  ...
  container.innerHTML += `
    <div id="${typingId}" class="chat-message">
      <div class="msg-avatar">${currentDmUser.emoji}</div>
      <div class="msg-body">
  ...
  // AI Reply
  try {
    const sysPrompt = `You are playing the role of "${currentDmUser.name}", a user on the Slam Trainers Network app. You're chatting privately with another user. Be casual, act like a real person on a hookup/fitness app. Keep your responses short, 1-2 sentences maximum. Don't break character.`;
    const msgs = [{ role: 'system', content: sysPrompt }];
    
    dmHistories[currentDmUser.name].slice(-6).forEach(m => {
      msgs.push({ role: m.mine ? 'user' : 'assistant', content: m.text || '[Image sent by user]' });
    });
    ...
    dmHistories[currentDmUser.name].push({ text: reply, time: 'just now', mine: false });
    if (currentDmUser) renderDmMessages();
    if (typeof playSound === 'function') playSound('receive');
  } catch(err) {
    document.getElementById(typingId)?.remove();
    dmHistories[currentDmUser.name].push({ text: "Hey! Let's chat more later 🔥", time: 'just now', mine: false });
    if (currentDmUser) renderDmMessages();
  }
}
...
function handleDmPictureUpload(event) {
  ...
  dmHistories[currentDmUser.name].push({ image: url, time: 'just now', mine: true });
  renderDmMessages();
  if (typeof playSound === 'function') playSound('send');
  
  // Fake a reaction after a bit
  setTimeout(() => {
    if (!currentDmUser) return; // if left view
    const reactions = ["Damn 🔥", "Looking good 👀", "Woah 🥵", "Nice pic!"];
    const reaction = reactions[Math.floor(Math.random() * reactions.length)];
    dmHistories[currentDmUser.name].push({ text: reaction, time: 'just now', mine: false });
    renderDmMessages();
    if (typeof playSound === 'function') playSound('receive');
